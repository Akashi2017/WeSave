// Netlify Function - 公众号文章下载 API
// 部署后访问路径: /.netlify/functions/fetch
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== 'POST') {
    return { statusCode: 405, headers, body: JSON.stringify({ success: false, error: '仅支持 POST' }) };
  }

  try {
    const { url } = JSON.parse(event.body || '{}');
    if (!url || !url.includes('mp.weixin.qq.com')) {
      return { statusCode: 400, headers, body: JSON.stringify({ success: false, error: '请提供有效的公众号文章链接' }) };
    }
    ...
    if (html.includes('请在微信客户端打开')) {
      return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: '该文章需在微信中打开' }) };
    }

    const article = extractContent(html);
    if (!article.title && !article.body) {
      return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: '无法解析文章内容' }) };
    }
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        data: { title: article.title, account: article.account, publishTime: article.publishTime, content: article.body, markdown: md, html: cleanHtml, wordCount: wc }
      }),
    };
  } catch (e) {
    return { statusCode: 200, headers, body: JSON.stringify({ success: false, error: '获取失败: ' + e.message }) };
  }
};
...
function renderHtml(title, account, time, body) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${s(title)}</title><style>body{font-family:-apple-system,sans-serif;max-width:680px;margin:0 auto;padding:16px;line-height:1.7;color:#333}img{max-width:100%;height:auto;border-radius:4px}h1{font-size:22px}</style></head><body><h1>${s(title)}</h1><p style="color:#999">${s(account)}${time?' · '+time:''}</p>${body||'<p>（无正文）</p>'}</body></html>`;
}
